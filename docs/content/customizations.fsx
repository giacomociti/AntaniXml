...
the enclosing elements or global complex types. 
...
generators are used but the generated elements are then transformed with a custom function.
...
The element generated may look like this:
    ...
    <foo>
      <bar id="FX(-K䤊Q&#xA;N(7RIS.&quot;. />
      <baz>j慬X"bL@V'yQy[y9O~r~rTH2kwkwkMA+dzdЁdz:$]s</baz>
    </foo>
 
Yes, text is gibberish (but for testing purposes usually it's a good idea to probe a system
with strange data), anyway you see the attribute value is all upper case thanks to the customization.
...
for transforming the generated elements.
...
(**
    In the example above instead we are creating a generator from scratch using the FsCheck combinators.
    ...
    <foo>
      <bar id="ZDZB}B}BS=S５v;v;L玲o4o4o&#xA;E/h~h~h(&gt;(awawa!7!ZpZpZu0:SiSiSn)bLbLbLv[/-&#x9;|cFh!;EBog7c" />
      <baz>c</baz>
    </foo>
